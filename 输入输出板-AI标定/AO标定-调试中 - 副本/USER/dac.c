...
{  
  ...
  RCC_AHB1PeriphClockCmd(AnalogOutput_RCC, ENABLE);//使能GPIOA时钟
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_DAC, ENABLE);//使能DAC时钟
	   
  GPIO_InitStructure.GPIO_Pin = AnalogOutput_pin1|AnalogOutput_pin2;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AN;//模拟输入
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_DOWN;//下拉
  GPIO_Init(AnalogOutput_Port, &GPIO_InitStructure);//初始化
...
void AnalogOutput_SetBy_Vol(u8 ch,u16 vol)
{
	...
	switch(ch)
	{
		case DAC_CH1:	DAC_SetChannel1Data(DAC_Align_12b_R,temp);//12位右对齐数据格式设置DAC值
			break;
		case DAC_CH2:	DAC_SetChannel2Data(DAC_Align_12b_R,temp);//12位右对齐数据格式设置DAC值
			break;
	}
}
//设置通道1输出电压
